...
export const SubidaImagen = ({selectedImages,setSelectedImages,onSelectFile,onDeleteFile}) => {
  return (
            ...
            {selectedImages && selectedImages.map((image,index)=>{
                return (
                    <div className='relative w-fit' key={image.id}>
                        <img className='rounded-md' src={image.id} width="300" alt="imagen" />
                        ...
                        {/* <li className='absolute top-3 right-3 w-1 h-1 p-3 bg-color-principal rounded-full flex items-center justify-center  duration-300 text-white' onClick={()=>setSelectedImages(selectedImages.filter(e=>e!==image))}><i className="fa-sharp fa-solid fa-xmark"></i></li> */}
                        <span className='cursor-pointer absolute top-3 right-3 w-1 h-1 p-3 bg-color-principal rounded-full flex items-center justify-center  duration-300 text-white' onClick={()=>{onDeleteFile(image)}}><i className="fa-sharp fa-solid fa-xmark"></i></span>
                    </div>
                )
            })}
        </div>
